...
| `sub_group_max_depth` | integer | `>=1` | no | `sub_import_groups=1` | Max nesting depth when rebuilding imported groups; a member that would exceed it (or re-enter a group already on the build stack) is dropped so the config still passes `sing-box check`. Defaults to `3`. Read by `outbound.uc`. |
