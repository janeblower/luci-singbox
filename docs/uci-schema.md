...
**Ownership of the transparent path.** `tproxy.nft_rules` and `tun.auto_route` both claim system routing / the firewall, and exactly one may. `helpers.transparent_claims` / `helpers.transparent_conflict` are the single place that decides; `generate.uc` refuses to build a config where both claim it (and bails before writing `/tmp/singbox-ui.json`, so the last known-good config survives) with exit code 3 (`EXIT_TRANSPARENT_CONFLICT`), and `init.d` refuses to start ONLY on that specific rc. The UI disables the loser's checkbox, so the conflict is only reachable by hand-editing UCI. Every OTHER `generate.uc` failure (e.g. a transient config-write failure, which also keeps the previous good config in place) exits 1, and `init.d` deliberately does NOT treat that as a refusal — it falls through to its own guards (`[ ! -s /tmp/singbox-ui.json ]` and `sing-box check`) and starts on the stale-but-valid config rather than take sing-box down over a disk hiccup on a 15-minute cron reload.

**A conflict must never tear the firewall down.** `reload` (both `*/15` cron jobs and the UI's Apply) is `stop` + `start`, and `stop` removes our `inet singbox_ui` table — so refusing inside `start` meant the table was already gone and the LAN egressed unproxied. `reload_service` therefore runs `generate.uc check-conflict` (the guard alone, same rc 3, nothing built or written) BEFORE `stop`, and on rc 3 aborts the reload without touching the running daemon: the last known-good config keeps serving, rules up, and only the operator's bad edit is refused. Any other rc from the check falls through to the normal `stop`+`start` (fail open — a broken checker must not become a broken reload). Boot is the exception: `start_service` has nothing to preserve, so it still refuses outright.
...
| `sub_import_groups` | bool | `0`/`1` | no | `sub_multi=1` | When `1`, the provider's own `urltest`/`selector` proxy groups are rebuilt as namespaced outbounds tagged `<sub>__grp__<slug>` whose members resolve **strictly within the subscription** (to imported nodes or other imported groups), and `main` replaces the provider's top selector. Default off → behaves exactly as before (flat selector over all nodes). Read by `outbound.uc`. |
| `sub_hide_grouped_nodes` | bool | `0`/`1` | no | `sub_import_groups=1` | When `1` (default), leaf nodes claimed by an imported group are removed from the top-level `main` selector's children (they stay reachable inside their group and remain emitted in `outbounds`). View-only; never drops a node from the config. Read by `outbound.uc`. |
| `sub_trust_provider` | bool | `0`/`1` | no | `sub_import_groups=1` | When `1`, an imported group member/`default` naming the builtin `direct` or `block` is allowed to resolve to it; otherwise such members are dropped (a group may only reference this subscription's own nodes/groups). Default off. Read by `outbound.uc`. |
| `sub_group_max_depth` | integer | `>=1` | no | `sub_import_groups=1` | Max group-nesting depth when rebuilding imported groups. Members are resolved by building the referenced group depth-first, so a member group that would nest deeper than this limit (or re-enter a group already on the build stack — a cycle) is dropped from that parent, keeping the config acyclic and bounded so it still passes `sing-box check`. Such a group can still appear at a shallower level or as a top-level child. Defaults to `3`. Read by `outbound.uc`. |
| `sub_cache_persist` | bool | `0`/`1` | no | `type=subscription` | Default `1`: keep a last-known-good copy of this subscription's node list on flash (`/etc/singbox-ui/sub-cache/`) so it survives a reboot. `0` keeps it in tmpfs only — no flash write, no flash restore — for subscriptions whose lines carry node passwords the user does not want on flash, and to save flash wear. Read by `subscription.uc`. |
