...
IMG_HASH=$(cat tests/browser-container/Dockerfile \
               ...
               tests/browser-container/uhttpd.conf 2>/dev/null \
               | sha256sum | cut -c1-12)
...
echo "==> launching container $CNAME on port $PORT"
...
# Wait for HTTP ready (max 30s).
i=0
while [ "$(curl -s -o /dev/null -w '%{http_code}' \
              "http://127.0.0.1:${PORT}/cgi-bin/luci" 2>/dev/null || echo 000)" = "000" ]; do
    i=$((i+1))
    if [ $i -gt 30 ]; then
        echo "FAIL: container not responding after 30s" >&2
