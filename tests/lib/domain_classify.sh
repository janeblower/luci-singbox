...
#   bbolt:     ^bbolt-client/
#   backend:   ^(singbox-ui/|luci-app-singbox-plugin-awg-warp/root/|tests/backend/|tests/parity/)
#   ui:        ^(luci-app-singbox-ui/|luci-app-singbox-plugin-awg-warp/htdocs/|tests/ui/|tests/browser/)
...
sb_classify_domains() {
	...
	if [ "$_github_shared" = true ] || printf '%s\n' "$_in" | grep -qE '^(tests/lib/|tests/run|tests/docker/|tests/browser-container/)'; then
		...
		printf '%s\n' "$_in" | grep -qE '^(singbox-ui/|luci-app-singbox-plugin-awg-warp/root/|tests/backend/|tests/parity/)' && _backend=true
		printf '%s\n' "$_in" | grep -qE '^(luci-app-singbox-ui/|luci-app-singbox-plugin-awg-warp/htdocs/|tests/ui/|tests/browser/)' && _ui=true
