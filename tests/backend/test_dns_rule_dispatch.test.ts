...
let refs = dns.referenced_rulesets({ rules: dns.build_rules(cur) });
...
`;
  ...
  // referenced_rulesets used to walk UCI a SECOND time instead of reading the
  // built rules, and the two walks had already drifted: build_rules drops an
  // action=route rule whose server is disabled BEFORE resolving its rule-sets,
  // that walk did not. Result: a rule-set kept alive — emitted into
  // route.rule_set, downloaded, refreshed on a schedule — for a dns rule that is
  // not in the config at all.
  it("dns.referenced_rulesets: a rule dropped for a disabled server keeps no rule-set", async () => {
    const src = `
let dns = require("dns");
let CFG = {
  dns_server: [ { [".name"]: "live", enabled: "1", type: "udp" },
                { [".name"]: "off",  enabled: "0", type: "udp" } ],
  ruleset:    [ { [".name"]: "rs_dead", enabled: "1" }, { [".name"]: "rs_live", enabled: "1" } ],
  dns_rule: [
    { [".name"]: "dead", enabled: "1", type: "default", domain: [ "x" ],
      rule_set: [ "rs_dead" ], action: "route", server: "off" },
    { [".name"]: "live", enabled: "1", type: "default", domain: [ "y" ],
      rule_set: [ "rs_live" ], action: "route", server: "live" },
  ],
};
let cur = {
  foreach: function(_p, t, fn) { for (let s in (CFG[t] || [])) fn(s); },
...
let refs = dns.referenced_rulesets({ rules: dns.build_rules(cur) });
if (length(refs) != 1 || refs[0] != "rs_live") { print(sprintf("FAIL: expected [rs_live], got %J\\n", refs)); exit(1); }
print("OK\\n");
`;
    const r = await runUcode(src);
    expect(r.exitCode).toBe(0);
