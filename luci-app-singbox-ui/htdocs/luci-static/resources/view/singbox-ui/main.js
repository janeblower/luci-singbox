...
return view.extend({
	...
	render: function () {
		...
		.then(function (nodes) {
			...
			// This MUST be scheduled before the mode-switcher branch below returns:
			// it wires the Advanced view, which the switcher only wraps. Scheduling it
			// after the early return meant that as soon as ANY plugin exposed a mode(),
			// all 8 tabs rendered stacked and visible, tab headers did nothing, the
			// status panel stayed empty and Monitoring/Dashboard never polled (F-40).
