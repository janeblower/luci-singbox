...
JS_DIR="$ROOT/luci-app-singbox-ui/htdocs/luci-static/resources/view/singbox-ui"
PO_DIR="$ROOT/luci-app-singbox-ui/po"
POT="$PO_DIR/templates/luci-app-singbox-ui.pot"
RU="$PO_DIR/ru/luci-app-singbox-ui.po"
...
find "$JS_DIR" -name '*.js' -print0 \
    | xargs -0 xgettext \
        ...
        --keyword=_ \
        --from-code=UTF-8 \
        --package-name=luci-app-singbox-ui \
        -o "$POT"

if [ -f "$RU" ]; then
    msgmerge --update --backup=none --no-fuzzy-matching "$RU" "$POT"
else
    echo "po/ru/luci-app-singbox-ui.po missing — initialize manually" >&2
    exit 1
fi
